...
class EquivalentXml
  ...
  attr_accessor :diff; private :diff=
  ...
  # @param [Nokogiri::XML::Node, Nokogiri::XML::NodeSet] node_2 The second top-level XML node to compare
  ...
  def compare_attributes(node_1, node_2, opts, &block)
    (node_1.name == node_2.name) && (node_1.value == node_2.value).tap do |same|
      self.diff << generate_xml_diff_string([node_1.path, node_2.path], [node_1.value, node_2.value]) unless same
    end
  end

  def compare_text(node_1, node_2, opts, &block)
    text_1, text_2 = [node_1, node_2].map(&:text)
    text_1, text_2 = [text_1, text_2].map{ |t| t.strip.gsub(/\s+/,' ') } if opts[:normalize_whitespace]
    (text_1 == text_2).tap do |same|
      self.diff << generate_xml_diff_string([node_1.path, node_2.path], [text_1, text_2]) unless same
    end
  end

  def compare_cdata(node_1, node_2, opts, &block)
    node_1.text == node_2.text.tap do |same|
      self.diff << generate_xml_diff_string([node_1.path, node_2.path], [node_1.text, node_2.text]) unless same
    end
  end
  ...
  def compare_nodesets(nodeset_1, nodeset_2, opts, &block)
    ...
    if local_set_1.length != local_set_2.length
      expected_but_missing, present_but_not_expected = [local_set_1, local_set_2].map{ |s| "\n#{s.map(&:path)*"\n"}" }
      self.diff << generate_xml_diff_string([expected_but_missing, present_but_not_expected], ['', ''])
      return false
    end
    ...
    local_set_1.each do |search_node|
      ...
      if found_node.nil?
        ...
        self.diff << generate_xml_diff_string([search_node.path, msg], [search_node.content, msg])
        return false
      else
  ...
  def generate_xml_diff_string(paths, texts)
    delimiters = %W(- +).map { |c| "\n#{c} " }
    [[paths, 'PATH'], [texts, 'TEXT']].map do |values, tag|
      delimiters.zip(values).map { |delimiter, value| "#{delimiter}#{tag}: #{value.gsub("\n", delimiter)}" }
